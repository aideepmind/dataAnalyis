...
//			}
	 ...
	 * @return
	 */
	public static Account getCurAccount(HttpServletRequest request) {
		Account a = null;
	 ...
	 * @return
	 */
	public Object getAuth(HttpServletRequest request, String role) {
		Object val = CurrentAccount.getAuth(request, role);
		//可以验证st的有效性
		//验证
		//失效的话，需要获取新的搜索凭证
//		if (val == null) {
...
//			if (a != null) {
//				String url = null;
//				if (Role.VALUE_STU_INFO_COMPLETE.equals(role)) {
//					url = Constant.systemUrlAuthSso + "/auth/getStuInfoCompleteToken?accessToken=" + a.getAccessToken() + "&ip=" + HttpUtil.getIpAddr(request);
//				} else if (Role.VALUE_COMP_OPEN_SEARCH.equals(role)) {
//					url = Constant.systemUrlAuthSso + "/auth/getCompSearchToken?accessToken=" + a.getAccessToken() + "&ip=" + HttpUtil.getIpAddr(request);
//				} else if (Role.VALUE_COMP_OPEN_SERVICE.equals(role)) {
//					url = Constant.systemUrlAuthSso + "/auth/getCompOpenServiceToken?accessToken=" + a.getAccessToken() + "&ip=" + HttpUtil.getIpAddr(request);
//				} else if (Role.VALUE_COMP_INFO_COMPLETE.equals(role)) {
//					url = Constant.systemUrlAuthSso + "/auth/getCompInfoCompleteToken?accessToken=" + a.getAccessToken() + "&ip=" + HttpUtil.getIpAddr(request);
//				}
//				if (url != null) {
//					String content = HttpUtil.sendGetRequest(url);
//					if (content != null && !"".equals(content)) {
//						ResponsePojo rp = (ResponsePojo) JSONObject.toBean(JSONObject.fromObject(content), ResponsePojo.class);
//						if (rp.isSuccess()) {
//							val = rp.getMessage();
//							CurrentAccount.addAuth(request, role, val);
//						}
//					}
//				}
//			}
//		}
	 ...
	 * @param role
	 */
	public static void refreshAuth(HttpServletRequest request, String role) {
		Account a = getCurAccount(request);
//		if (a != null) {
//			String url = null;
//			if (Role.VALUE_STU_INFO_COMPLETE.equals(role)) {
...
//		}
	 ...
	 * @param request
	 */
	public static void refreshAllAuth(HttpServletRequest request) {
		Map<String, Object> m = CurrentAccount.getAllAuth(request);
		if (m != null) {
			Iterator<String> it = m.keySet().iterator();
			while (it.hasNext()) {
				refreshAuth(request, it.next());
			}
		}
	}
}
